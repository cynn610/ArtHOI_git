...
def _script_runtime(script_path, kwargs):
    """Resolve a portable cwd/PYTHONPATH for a third-party script."""
    script_path = os.path.abspath(script_path)
    script_dir = os.path.dirname(script_path)
    runtime_env = dict(os.environ)
    runtime_env.update(kwargs.pop("env", {}) or {})
    python_entries = [PROJECT_ROOT, script_dir, os.path.dirname(script_dir)]
    inherited = runtime_env.get("PYTHONPATH")
    if inherited:
        python_entries.append(inherited)
    runtime_env["PYTHONPATH"] = os.pathsep.join(python_entries)
    kwargs.setdefault("cwd", script_dir)
    kwargs["env"] = runtime_env
    return script_path, kwargs
...
def call_script_block(script_path, args_list, conda_env=None, **kwargs):
    ...
    """
    script_path, kwargs = _script_runtime(script_path, kwargs)
    cmd = [sys.executable, script_path] + args_list
    if conda_env and conda_env != "current":
        cmd = ['conda', 'run', '-n', conda_env, '--live-stream',
               'python', script_path] + args_list
    loguru.info(f"Executing command: {shlex.join(cmd)}")
...
def call_script_async(script_path, args_list, conda_env=None, **kwargs):
    ...
    if conda_env and conda_env != "current":
        cmd = ['conda', 'run', '-n', conda_env, '--live-stream',
               'python', script_path] + args_list
    loguru.debug(f"Executing command asynchronously: {shlex.join(cmd)}")
