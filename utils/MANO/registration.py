...
def fit_mano(
    ...
    sem_idx_path = f"{MANO_DIR}/sealed_vertices_sem_idx.npy"
    sealed_vertices_sem_idx = None
    if sem_idx and os.path.exists(sem_idx_path):
        sealed_vertices_sem_idx = np.load(sem_idx_path, allow_pickle=True)
    params = []
    if "rot" in optim_on:
        params.append(rot)
    ...
    for i in pbar:
        ...
        sloss = (
            sem_loss(hand_verts, target_vertices, sealed_vertices_sem_idx, sem_idx)
            if sealed_vertices_sem_idx is not None
            else 0.0
        )
