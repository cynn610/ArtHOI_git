...
from hrse.hand_instances import contact_tracks, hand_instance_map, xyz_pullback_instances
...
def compute_loss(
    ...
    if 'projection_error' in enabled:
        ...
        hand_proj_err = torch.zeros(1, device=device)
        for k in pjerr_keys:
        ...
        # Keep a small, differentiable image-space anchor for the hand.  The
        # old code converted this term to a Python float, so it affected only
        # the displayed total and supplied no gradient to MANO translation or
        # pose. After MANO registration, this stabilizes lateral corrections
        # without overpowering the 3-D contact term.
        hand_proj_weight = getattr(args.ho_align, 'hand_projection_weight', 0.05)
        if hand_proj_weight > 0:
            loss_dict['hand_projection'] = hand_proj_err * hand_proj_weight
    
    if 'contact_dis' in enabled:
        contact_err = {
            instance_id: [torch.tensor(0.0, device=device)]
            for instance_id in sides
        }
        for hand_side in sides:
            for f in range(F):
                if contacts[hand_side][f]:
                    active_fingers = [
                        finger
                        for finger in contacts[f"{hand_side}_fingers"][f]
                        if finger not in ("pinky", "ring")
                    ]
        ...
        for instance_id, values in contact_err.items():
            loss_dict[f'contact/{instance_id}'] = torch.stack(values, dim=0).mean()
    ...
    all_loss = torch.zeros((), device=device)
    for k, v in loss_dict.items():
        if not torch.is_tensor(v):
            v = torch.tensor(v, dtype=all_loss.dtype, device=device)
        all_loss = all_loss + v.squeeze()
    if writer is not None:
...
def penetration_stage(
    ...
    Loss = pen_repulsion (min_dist, gradients bring the nearest hand surface to
           penetrating vertices so they can cross out of the closed mesh)
    ...
    for step in (pbar := tqdm(range(pen_steps), desc="Penetration stage")):
        ...
        # For object vertices detected inside the hand, minimize their distance
        # to the nearest hand-surface vertex.  With the mask refreshed below,
        # points drop out of the loss as soon as they cross the closed surface.
...
def refined_opt(
    ...
    xyz_instances = xyz_pullback_instances(args.ho_align, sides)
    if xyz_instances:
        loguru.info(f"XYZ pull-back enabled for hand instances: {sorted(xyz_instances)}")
    ...
    for step in (pbar := tqdm(range(args.ho_align.steps))):
        ...
        for i in range(args.part_cnt):
            ...
            for f in range(F):
                ...
                v3d.append(vp)
                v2d.append(project_points(vp, K, w2c, pt3d_coord=True))
        ...
        for s in sides:
            ...
            for f in range(F):
                ...
                if s in xyz_instances:
                    # A bad left-hand initializer can be displaced laterally
                    # as well as in depth. Use the complete residual vector;
                    # the legacy path intentionally kept only z adjustable.
                    hx = hx + res_h[s].translation
                    jx = jx + res_h[s].translation
                else:
                    z_only = torch.stack((
                        torch.zeros_like(res_h[s].translation[0]),
                        torch.zeros_like(res_h[s].translation[1]),
                        res_h[s].translation[2],
                    ))
                    hx = hx + z_only
                    jx = jx + z_only
                # hj = res_h[s].forward(hj3d_gt[s][f])
                hv3d_p.append(hx)
                hv2d_p.append(project_points(hx, K, w2c, pt3d_coord=True))
                hj3d_p.append(jx)
                hj2d_p.append(project_points(jx, K, w2c, pt3d_coord=True))
            preds[s] = {
        ...
        if step == 150 or step == 400:
            ...
            if step == 400:
                ...
                if getattr(args.ho_align, 'hard_velo', False):
...
def main():
    ...
    parser.add_argument(
        '--contact-path', type=str, default=None,
        help='Contact annotation JSON; defaults to <seq-path>/processed/ho_contact.json',
    )
    ...
    parser.add_argument(
        ...
        help=(
            'Render and export the supplied hand/object priors without running '
            'the alignment optimizer. Useful for calibrated ground-truth adapters.'
        ),
    )
    ...
    contact_path = args.contact_path or f'{args.seq_path}/processed/ho_contact.json'
    with open(contact_path, 'r') as f:
        contact_data = json.load(f)
    instances = hand_instance_map(contact_data)
    appeared = list(instances)
    contacts = contact_tracks(contact_data, F, instances)
    ...
    hand_avails = {}
    for instance_id, handedness in instances.items():
        h_param = mano_params[instance_id]
        hand_avails[instance_id] = h_param["is_valid"].astype(np.int8)
        ## solve mean pose
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            model_io.dump_ho_data(hand_params, parts_tfs, parts, args)
        return
