...
class MANOParams(nn.Module):
    ...
    def __init__(self, mano_params, hand_type,
        ...
        if hand_type not in ("left", "right"):
            raise ValueError(f"MANO handedness must be left/right, got {hand_type!r}")
        self.handedness = hand_type
        self.flat_hand_mean = bool(mano_params.get('flat_hand_mean', False))
        self.device = device
        ...
        self.K = torch.as_tensor(K, dtype=torch.float32, device=self.device)
        self.w2c = torch.as_tensor(w2c, dtype=torch.float32, device=self.device)
