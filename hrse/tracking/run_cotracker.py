...
from hrse.tracking.tracking_utils import (
...
COTRACKER_ROOT = Path(__file__).resolve().parents[2] / "third_party" / "co-tracker"
COTRACKER_CHECKPOINT_DIR = COTRACKER_ROOT / "checkpoints"


def load_cotracker(ckpt="cotracker3_offline"):
    """Load CoTracker model"""
    loguru.info(f"Loading CoTracker model: {ckpt}")
    checkpoint = COTRACKER_CHECKPOINT_DIR / "scaled_offline.pth"
    if not checkpoint.is_file():
        raise FileNotFoundError(f"Missing CoTracker checkpoint: {checkpoint}")
    model = CoTrackerPredictor(checkpoint=str(checkpoint), window_len=60, v2=False)
    model.eval()
...
def load_cotracker_online(ckpt="cotracker3_online"):
    ...
    loguru.info(f"Loading CoTracker model: {ckpt}")
    checkpoint = COTRACKER_CHECKPOINT_DIR / "scaled_online.pth"
    if not checkpoint.is_file():
        raise FileNotFoundError(f"Missing CoTracker checkpoint: {checkpoint}")
    model = CoTrackerOnlinePredictor(checkpoint=str(checkpoint), window_len=16, v2=False)
    model.eval()
    loguru.success("CoTracker model loaded successfully")
    return model
