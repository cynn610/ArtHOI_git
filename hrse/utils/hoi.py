...
def render_occupancy(
    ...
    meshes = [obj_mesh]
    meshes.extend(hand_meshes.values())
...
def deform_HO(
    ...
    for i in range(len(parts)):
        ...
        for f in ids:
            ...
            v3d.append(vp)
            v2d.append(image_utils.project_points(vp, K, w2c, pt3d_coord=True))
    ...
    for s in hp.keys():
        ...
        if g_handtf is not None and s in g_handtf:
            ...
            hand_dict['v2d'] = image_utils.project_points(
                hand_dict['v3d'], K, w2c, pt3d_coord=True
            )
            hand_dict['j2d'] = image_utils.project_points(
                hand_dict['j3d'], K, w2c, pt3d_coord=True
            )
        ret[s] = hand_dict
    ## debug
