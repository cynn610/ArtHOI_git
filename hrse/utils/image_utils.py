...
def project_points(points, K, extrin, pt3d_coord=False):
    """
    param points: [BS, N, 3]
    param K: [3, 3]
    param extrin: [4, 4]
    ...
    _K[:3, :3] = K[:3, :3]
    if pt3d_coord:
        # PyTorch3D camera space is +X left, +Y up, while pixel/OpenCV camera
        # space is +X right, +Y down. Convert after w2c and before applying K.
        reverse = torch.eye(4, dtype=K.dtype, device=K.device)
        reverse[0, 0] = -1.0
        reverse[1, 1] = -1.0
        P = torch.matmul(_K, torch.matmul(reverse, extrin))
    else:
