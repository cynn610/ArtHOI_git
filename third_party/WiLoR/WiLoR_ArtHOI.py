...
def main():
    ...
    parser.add_argument(
        '--model-dir',
        default='wilor_af',
        help='ArtHOI processed subdirectory used for outputs.',
    )
    ...
    args.out_folder = args.img_folder.replace(
        '/build/image', f'/processed/{args.model_dir}'
    )
    ...
    for img_path in tqdm(img_paths):
        ...
        for batch in dataloader: 
            ...
            for n in range(batch_size):
                ...
                mano_param['global_orient'] = matrix_to_axis_angle(
                    out['pred_mano_params']['global_orient'][n]).cpu().numpy().squeeze(0)
                # hand_pose (Bs, 15, 3, 3) -> (45,)
                mano_param['hand_pose'] = matrix_to_axis_angle(
