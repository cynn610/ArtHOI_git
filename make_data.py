...
# MAIN Conda env
AHOI_ENV = os.environ.get("ARTHOI_ENV", "arthoi")
# Standalone envs
DIFFUERASER_ENV = os.environ.get("ARTHOI_DIFFUERASER_ENV", AHOI_ENV)
WILOR_ENV = os.environ.get("ARTHOI_WILOR_ENV", AHOI_ENV)
PARTFIELD_ENV = os.environ.get("ARTHOI_PARTFIELD_ENV", AHOI_ENV)
...
def get_args():
    ...
    parser.add_argument('--out', type=str, default=f'{ROOT}/workspace/sequences/',
                        help='Output directory for the generated sequence')
    parser.add_argument(
        '--conf-out', type=str, default=None,
        help='Generated sequence config path (default: <out>/../configs/<sequence>.yml)',
    )
    ...
    parser.add_argument(
        '--skip-partfield', action='store_true',
        help='Skip PartField features/separation; use downstream 2D-mask separation',
    )
    parser.add_argument('--target-reso', '--target_reso', type=int, nargs=2, default=(960, 540),
...
def Object_recon(seq_path, seq_name, args):
    ...
    canonical_obj = os.path.join(obj_path, f'{cano_frame:05d}.obj')
    if not os.path.isfile(canonical_obj):
        raise FileNotFoundError(
            f"Canonical object mesh is missing: {canonical_obj}. "
            "Run Hunyuan3D or place the canonical GLB under build/mesh/raw and resume."
        )
    loguru.info(f"Processed {len(glb_files)} GLB files to OBJ with vertex colors in {glb_path}")
    
    # Get Object PartField pre-calculation and candidate clusters.
    if args.skip_partfield:
        loguru.warning(
            "Skipping PartField; canonical parts must be separated with the 2D-mask path."
        )
    else:
        ...
        if not skip:
            part_sep_args = [
                "--root", f"{PARTFIELD_ROOT}/exp_results/partfield_features/{seq_name}/",
                "--source_model", f"{seq_path}/build/mesh/{cano_frame:05d}.obj",
                "--dump_dir", f"{seq_path}/processed/partseps/",
                "--conf", args.conf_out,
            ]
            file_utils.call_script_block(
                f'{PARTFIELD_ROOT}/partfield/run_seperate.py',
...
def generate_conf(args, seq_name):
    ...
    conf_template['PF_cluster_cnt'] = max(2 * part_cnt, 2)
    default_conf_dir = os.path.join(os.path.dirname(args.out), 'configs')
    conf_out = args.conf_out or os.path.join(
        default_conf_dir, f'{seq_name}_p{part_cnt}c{cano_frame}.yml'
    )
    conf_out = os.path.abspath(conf_out)
    os.makedirs(os.path.dirname(conf_out), exist_ok=True)

    if os.path.isfile(conf_out):
        with open(conf_out, 'r', encoding='utf-8') as f:
            existing = yaml.load(f) or {}
        expected = {
            'part_cnt': part_cnt,
            'cano_frame': cano_frame,
        }
        mismatched = {
        ...
        if mismatched:
            raise ValueError(
                f"Existing config does not match the requested sequence: {mismatched}"
            )
        if not args.skip_partfield and 'PF_cluster_cnt' not in existing:
            raise ValueError(
                f"Existing config is missing required PF_cluster_cnt: {conf_out}"
            )
        loguru.info(f"Reusing existing sequence config without overwriting it: {conf_out}")
        return conf_out
...
def main():
    ...
    if args.part_cnt < 1:
        raise ValueError("--part-cnt must be at least 1")
    if args.cano_frame < 0:
        raise ValueError("--cano-frame must be non-negative")
    ...
    args.conf_out = generate_conf(args, seq_name)
    skip_priors = ask_to_continue(
        "All video data preprocessing is done. "
